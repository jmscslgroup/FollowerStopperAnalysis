...
clc
clf
clear follower_xpos follower_ypos follower_vel follower_acc
...
t = xpos.time;
lead_xpos = xpos.signals(1).values;
lead_ypos = ypos.signals(1).values;
...
for n = 1:1:length(lead_xpos)
    axis([min(follower_xpos(:,9))-0.5 max(lead_xpos)+0.5 y1 y2])
